  ...
  return new Promise((resolve, reject) => {
    ...
    sock.on('data', (chunk) => {
      ...
      if (!shook) {
        ...
        shook = true;
        // The socket timeout only protects the HTTP/WebSocket
        // handshake. Leaving it armed kills an otherwise healthy target
        // while a second instance is still starting up.
        sock.setTimeout(0);
        resolve(api);
      }
      while (buf.length >= 2) {
...
export async function launch(port, extraArgs = []) {
  ...
  const proc = spawn(bin, [
    ...
    // test/lib/harness.mjs for why a GPU-less headless
