...
pub use audio::{
    play_ambient, play_music, play_sfx, stop_ambient, stop_music, BlipSound, MusicTracks,
};
pub use color::{
...
/// Embed a PNG generated by the game's `build.rs` under `$OUT_DIR/assets/images/`.
/// Example: `const HEAD_PNG: &[u8] = blip_image!("head.png");`
#[macro_export]
macro_rules! blip_image {
    ($name:literal) => {
        include_bytes!(concat!(env!("OUT_DIR"), "/assets/images/", $name))
    };
}

/// Embed a WAV generated by the game's `build.rs` under `$OUT_DIR/assets/sounds/`.
/// Example: `const EAT_WAV: &[u8] = blip_sound!("eat.wav");`
#[macro_export]
macro_rules! blip_sound {
    ($name:literal) => {
        include_bytes!(concat!(env!("OUT_DIR"), "/assets/sounds/", $name))
    };
}
